        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-500">
              {totalCount > 0 && !isLoading ? `${totalCount.toLocaleString()} verified ${totalCount === 1 ? 'exporter' : 'exporters'} listed` : ''}
            </p>
          </div>
          ...
          <div className="max-w-2xl mx-auto mb-8 flex gap-2">
            ...
            {hasFilters && (
              <Button variant="outline" onClick={clearSearch} className="h-11 px-4 text-sm">
                Clear
              </Button>
            )}
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-6 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 px-4 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <Link href="/login?returnUrl=/directory">
                <Button size="sm" variant="outline" className="border-green-600 text-green-700 hover:bg-green-50 dark:text-green-400 dark:border-green-600">
                  Log In
                </Button>
              </Link>
              <Link href="/register">
                        ...
                        >
                          <td className="px-5 py-4">
                            ...
                            {biz.verificationStatus === 'VERIFIED' && (
                              <Badge className="mt-1 text-[10px] bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0">
                                ✓ Verified
                              </Badge>
                            )}
                          </td>
                            ...
                            {biz.products && biz.products.length > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {biz.products.slice(0, 3).map(p => (
                                  <span key={p.id} className="inline-block bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs px-2 py-0.5 rounded-full">
                                    {p.name}
                                  </span>
                                ))}
                                {biz.products.length > 3 && (
                                  <span className="text-xs text-gray-400">+{biz.products.length - 3} more</span>
                                )}
                              </div>
                            ) : biz.serviceOffering ? (
                          ...
                          <td className="px-5 py-4 text-gray-600 dark:text-gray-400 text-xs leading-relaxed">
                            {[biz.physicalAddress, biz.town, biz.county].filter(Boolean).join(', ') || biz.location || <span className="italic">—</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
